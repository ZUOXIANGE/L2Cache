...
public class CacheStampedeTests
{
    ...
    public async Task GetOrLoadAsync_ConcurrentCalls_WithLocks_ShouldHitSourceOnce()
    {
        ...
        {
            await Assert.That(r).IsEqualTo($"db_{key}");
        }

        // 2. 验证回源次数
        // 启用了锁机制，预期回源次数为 1
        Console.WriteLine($"Concurrent requests: {concurrentClients}");
        Console.WriteLine($"Actual Source Queries: {cacheService.QueryDataCount}");

        // 注意：由于分布式锁的实现依赖于 Redis，如果环境延迟较高，偶尔可能会有多次，但通常应为1
        await Assert.That(cacheService.QueryDataCount).IsLessThan(3); // Allow small margin for error in test env
    }
}
